...
from bot_file.states.auth_state import AuthState
from bot_file.states.signin_state import SignInState
from bot_file.states.forgot_password_state import ForgotPasswordState
...
# @dp.message_handler(state=auth_state.user_login)
...
async def process_pass_password_sign_in(message: types.Message, state: FSMContext):
    if message.chat.type == 'private':
        async with state.proxy() as sign_in_data:
...
def get_password(username, password):
    ...
    if check_password(password, user.user_password):return True
    else:False
...
def authorization_handlers_register():
    ...
    dp.register_message_handler(process_sign_up, state=AuthState.user_login)
    ...
    dp.register_message_handler(process_pass_password_sign_in, state=SignInState.password)
